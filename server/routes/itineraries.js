...
router.get('/reminders/upcoming', requireUser, async(req, res) => {
    try {
        const userId = req.session.user.UserID;

        const [rows] = await pool.execute(`
      SELECT ItineraryID, title, start_date, end_date
      FROM Itinerary
      WHERE UserID = ?
...
router.get('/', requireUser, async(req, res) => {
    try {
        const [rows] = await pool.execute(
            'SELECT * FROM Itinerary WHERE UserID = ? ORDER BY start_date DESC', [req.session.user.UserID]
        );
        res.json(rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
...
router.get('/:id', requireUser, async(req, res) => {
    try {
        const [itineraries] = await pool.execute(
            'SELECT * FROM Itinerary WHERE ItineraryID = ? AND UserID = ?', [req.params.id, req.session.user.UserID]
        );
...
router.post('/', requireUser, async(req, res) => {
    try {
        ...
        const [result] = await pool.execute(
            'INSERT INTO Itinerary (title, start_date, end_date, UserID) VALUES (?, ?, ?, ?)', [title, start_date, end_date, req.session.user.UserID]
        );
...
router.delete('/:id', requireUser, async(req, res) => {
    try {
        const [result] = await pool.execute(
            'DELETE FROM Itinerary WHERE ItineraryID = ? AND UserID = ?', [req.params.id, req.session.user.UserID]
        );
...
router.post('/:id/items', requireUser, async(req, res) => {
    try {
        ...
        const [owns] = await pool.execute(
            'SELECT ItineraryID FROM Itinerary WHERE ItineraryID = ? AND UserID = ?', [itineraryId, req.session.user.UserID]
        );
        if (owns.length === 0) {
            return res.status(404).json({ error: 'Itinerary not found' });
        }
        ...
        const [maxItem] = await pool.execute(
            'SELECT COALESCE(MAX(ItemNo), 0) + 1 AS nextNo FROM ItineraryItem WHERE ItineraryID = ?', [itineraryId]
        );
        const itemNo = maxItem[0].nextNo;

        /* insert the item */
        await pool.execute(
            'INSERT INTO ItineraryItem (ItineraryID, ItemNo, planned_time, notes) VALUES (?, ?, ?, ?)', [itineraryId, itemNo, planned_time || null, notes || null]
        );

        /* if they picked a place, link it in the References table */
        if (placeId) {
            await pool.execute(
                'INSERT INTO `References` (PlaceID, ItineraryID, ItemNo) VALUES (?, ?, ?)', [placeId, itineraryId, itemNo]
            );
        }
...
router.put('/:id/items/:itemNo', requireUser, async(req, res) => {
    try {
        ...
        const [result] = await pool.execute(
            'UPDATE ItineraryItem SET planned_time = ?, notes = ? WHERE ItineraryID = ? AND ItemNo = ?', [planned_time || null, notes, req.params.id, req.params.itemNo]
        );
        if (result.affectedRows === 0) {
...
router.delete('/:id/items/:itemNo', requireUser, async(req, res) => {
    try {
        const [result] = await pool.execute(
            'DELETE FROM ItineraryItem WHERE ItineraryID = ? AND ItemNo = ?', [req.params.id, req.params.itemNo]
        );
        if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Item not found' });
        }
